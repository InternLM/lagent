...
```{attention}
`parameter_description` is usually inserted into the tool description automatically by the action's parser. It will be introduced in [Section](#interface-design) .
```
...
* `inputs`: Often a string in specific formats generated by LLMs. It depends on the action's parser.
