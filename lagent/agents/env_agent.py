...
from lagent.agents.fc_agent import EnvAgent
from lagent.hooks import Hook
from lagent.schema import ActionStatusCode, ActionValidCode, AgentMessage, AgentStatusCode
...
class RLEnvAgent(EnvAgent):
    def __init__(
        self,
        actions: list,
        judger: Union[Dict, AsyncAgent] = None,
    ...
    async def forward(self, assistant_message: AgentMessage, **kwargs):
        ...
        if not assistant_message.tool_calls:
            ...
            if self.judger is None:
                return AgentMessage(
                    sender=self.name, content="No Judger Configured", extra_info=extra_info, reward=0.0
                )
