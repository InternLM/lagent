...
OPENAI_STYLE_API_BASE = 'http://192.168.26.213:13000/v1/chat/completions'

def process_model_params(model_type, messages,gen_params,json_mode):
...
class GPTStyleAPI(BaseAPILLM):
    ...
    def __init__(
        ...
        stream: bool = False,
        api_base: str = OPENAI_STYLE_API_BASE,
        ...
        **gen_params,
        ...
        if isinstance(key, str):
            self.keys = [os.getenv('OPENAI_STYLE_API_KEY') if key == 'ENV' else key]
        else:
            self.keys = key
    ...
    def chat(
        ...
        **gen_params,
        ...
        gen_params = {**self.gen_params, **gen_params,"stream":self.stream}
    ...
    def generate_request_data(self, model_type, messages, gen_params, json_mode=False):
        ...
        # Model-specific processing
        data = process_model_params(model_type, messages, gen_params, json_mode)
...
class AsyncGPTStyleAPI(AsyncBaseAPILLM):
    """Model wrapper around OpenAI's models.
